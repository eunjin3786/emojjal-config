...
Emojjal(iOS) 앱의 **원격 설정** 저장소. 앱은 실행 시 `version.json`을 읽어 서비스 종료/강제 업데이트/공지를 제어한다.

- 앱이 읽는 주소(raw): `https://raw.githubusercontent.com/eunjin3786/emojjal-config/main/version.json`
...
- 모델: `Emojjal/Core/Models/AppUpdateConfig.swift`
- 체커: `Emojjal/Core/Services/AppUpdateChecker.swift` (`configURL`이 이 파일의 raw 주소)
- 화면: `Emojjal/UI/Components/AppUpdateGate.swift` — `.appUpdateGate()`를 `RootView`에 적용
